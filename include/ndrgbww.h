...
struct NDRGBWW {
    union {
        struct {
            ...
            union {
                uint8_t w1;     ///< white 1 channel value
                uint8_t white1;  ///< @copydoc b
            };
            union {
                uint8_t w2;     ///< white 2 channel value
                uint8_t white2;  ///< @copydoc b
            };            
        };
        ...
        /// * `raw[3]` is the white1 value
        /// * `raw[4]` is the white2 value        
        uint8_t raw[5];
    };
    ...
    inline NDRGBWW() __attribute__((always_inline)) = default;

    /// Allow construction from red, green, and blue
    /// @param ir input red value
    /// @param ig input green value
    /// @param ib input blue value
    inline NDRGBWW( uint8_t ir, uint8_t ig, uint8_t ib)  __attribute__((always_inline))
        : r(ir), g(ig), b(ib)
    {
    }
    ...
    /// @param ig input green value
    /// @param ib input blue value
    inline NDRGBWW( uint8_t ir, uint8_t ig, uint8_t ib, uint8_t iw1)  __attribute__((always_inline))
        : r(ir), g(ig), b(ib), w1(iw1)
    {
    }    
    ...
    /// @param ig input green value
    /// @param ib input blue value
    inline NDRGBWW( uint8_t ir, uint8_t ig, uint8_t ib, uint8_t iw1, uint8_t iw2)  __attribute__((always_inline))
        : r(ir), g(ig), b(ib), w1(iw1), w2(iw2)
    {
    }        

    /// Allow construction from 32-bit (really 24-bit) bit 0xRRGGBB color code
    /// @param colorcode a packed 24 bit color code
    inline NDRGBWW( uint32_t colorcode)  __attribute__((always_inline))
    : r((colorcode >> 24) & 0xFF), g((colorcode >> 16) & 0xFF), b((colorcode >> 8) & 0xFF), w1((colorcode >> 0) & 0xFF)
    {
    }

    /// Allow construction from a LEDColorCorrection enum
    /// @param colorcode an LEDColorCorrect enumeration value
    inline NDRGBWW( LEDColorCorrection colorcode) __attribute__((always_inline))
    ...
    inline NDRGBWW( ColorTemperature colorcode) __attribute__((always_inline))
    : r((colorcode >> 16) & 0xFF), g((colorcode >> 8) & 0xFF), b((colorcode >> 0) & 0xFF)
    {
    ...
    inline NDRGBWW(const NDRGBWW& rhs) __attribute__((always_inline)) = default;

    /// Allow assignment from one RGB struct to another
    inline NDRGBWW& operator= (const NDRGBWW& rhs) __attribute__((always_inline)) = default;

    /// Allow assignment from 32-bit (really 24-bit) 0xRRGGBB color code
    /// @param colorcode a packed 24 bit color code
    inline NDRGBWW& operator= (const uint32_t colorcode) __attribute__((always_inline))
    {
        r = (colorcode >> 24) & 0xFF;
        g = (colorcode >> 16) & 0xFF;
        b = (colorcode >>  8) & 0xFF;
    ...
    // Inline function to allow col2 = col1
    inline NDRGBWW& operator=(const CRGB& c) {
        r = c.r;
        g = c.g;
        b = c.b;
        w1 = 0;
        ...
        // Assuming you initialize w and ww somehow
        return *this;
    }
    ...
    inline NDRGBWW& setRGB (uint8_t nr, uint8_t ng, uint8_t nb) __attribute__((always_inline))
    {
        r = nr;
        g = ng;
        b = nb;
    ...
    /*
    inline NDRGBWW& setHSV (uint8_t hue, uint8_t sat, uint8_t val) __attribute__((always_inline))
    {
    ...
    /// @param hue color hue
    inline NDRGBWW& setHue (uint8_t hue) __attribute__((always_inline))
    {
    ...
    inline NDRGBWW& operator= (const CHSV& rhs) __attribute__((always_inline))
    {
    ...
    */
    ...
    /// Allow assignment from 32-bit (really 24-bit) 0xRRGGBB color code
    /// @param colorcode a packed 24 bit color code
    inline NDRGBWW& setColorCode (uint32_t colorcode) __attribute__((always_inline))
    {
        r = (colorcode >> 16) & 0xFF;
        g = (colorcode >>  8) & 0xFF;
        b = (colorcode >>  0) & 0xFF;
    ...
    inline NDRGBWW& operator+= (const NDRGBWW& rhs )
    {
        r = qadd8( r, rhs.r);
        g = qadd8( g, rhs.g);
        b = qadd8( b, rhs.b);
    ...
    inline NDRGBWW& addToRGB (uint8_t d )
    {
        r = qadd8( r, d);
        g = qadd8( g, d);
        b = qadd8( b, d);
    ...
    /// can't usefully decide when it's being passed a 32-bit
    /// constant (e.g. CRGB::Red) and an 8-bit one (CRGB::Blue)
    inline NDRGBWW& subtractFromRGB(uint8_t d )
    {
        r = qsub8( r, d);
        g = qsub8( g, d);
        b = qsub8( b, d);
    ...
    inline NDRGBWW& operator-- ()  __attribute__((always_inline))
    {
        subtractFromRGB(1);
        return *this;
    }
    ...
    /// @copydoc operator--
    inline NDRGBWW operator-- (int )  __attribute__((always_inline))
    {
        NDRGBWW retval(*this);
        --(*this);
        return retval;
    ...
    inline NDRGBWW& operator/= (uint8_t d )
    {
        r /= d;
        g /= d;
        b /= d;
    ...
    /// @see nscale8x3_video
    inline NDRGBWW& nscale8_video (uint8_t scaledown )
    {
    ...
    /// by "a percentage"
    inline NDRGBWW& operator%= (uint8_t scaledown )
    {
        nscale8x3_video( r, g, b, scaledown);
        return *this;
    }
    ...
    inline NDRGBWW& fadeLightBy (uint8_t fadefactor )
    {
    ...
    /// @see nscale8x3
    inline NDRGBWW& nscale8 (uint8_t scaledown )
    {
    ...
    /// levels may dim all the way to 100% black.
    /// @see ::scale8
    inline NDRGBWW& nscale8 (const NDRGBWW & scaledown )
    {
    ...
    inline NDRGBWW scale8 (uint8_t scaledown ) const
    {
        NDRGBWW out = *this;
    ...
    /// Return a CRGB object that is a scaled down version of this object
    inline NDRGBWW scale8 (const NDRGBWW & scaledown ) const
    {
        NDRGBWW out;
    ...
    inline NDRGBWW& fadeToBlackBy (uint8_t fadefactor )
    {
    ...
    inline NDRGBWW lerp8( const NDRGBWW& other, fract8 frac) const
    {
        NDRGBWW ret;
    ...
    /// @copydoc lerp8
    inline NDRGBWW lerp16( const NDRGBWW& other, fract16 frac) const
    {
        NDRGBWW ret;
...
inline __attribute__((always_inline)) bool operator== (const NDRGBWW& lhs, const NDRGBWW& rhs)
{
...
inline NDRGBWW operator+( const NDRGBWW& p1, const NDRGBWW& p2)
{
    return NDRGBWW( qadd8( p1.r, p2.r),
                 qadd8( p1.g, p2.g),
                 qadd8( p1.b, p2.b));
}
...
inline NDRGBWW operator-( const NDRGBWW& p1, const NDRGBWW& p2)
{
    return NDRGBWW( qsub8( p1.r, p2.r),
                 qsub8( p1.g, p2.g),
                 qsub8( p1.b, p2.b));
}
...
/// @copydoc CRGB::operator*=
__attribute__((always_inline))
inline NDRGBWW operator*( const NDRGBWW& p1, uint8_t d)
{
    return NDRGBWW( qmul8( p1.r, d),
                 qmul8( p1.g, d),
                 qmul8( p1.b, d));
}
...
inline NDRGBWW operator/( const NDRGBWW& p1, uint8_t d)
{
    return NDRGBWW( p1.r/d, p1.g/d, p1.b/d);
}
...
inline NDRGBWW operator&( const NDRGBWW& p1, const NDRGBWW& p2)
{
    return NDRGBWW( p1.r < p2.r ? p1.r : p2.r,
                 p1.g < p2.g ? p1.g : p2.g,
                 p1.b < p2.b ? p1.b : p2.b);
}
